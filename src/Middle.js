...
function Middle() {
  ...
  const post =  async (id) => {
    for(let x=0;x<2;x++){
    await getDocs(collection(db, id+"Middle"))
      .then((querySnapshot) => {
        const newData = querySnapshot.docs.map((doc) => ({...doc.data(), id: doc.id}));
        setShowDescription(newData);
        console.log(showDescription,newData);
        const newImg = newData.map((description,i)=>(
          getDownloadURL(ref(storage, id+"Middle"+'/'+description.title+'/image'))
            .then((url) => {
              if(description.image==''){
              updateDoc(doc(db, id+'Middle'+'/'+''+description.title), {
                image : url,
              });}
            }),
          getDownloadURL(ref(storage, id+"Middle"+'/'+description.title+'/pdf'))
            .then((url) => {
              if(description.pdf==''){
              updateDoc(doc(db, id+'Middle'+'/'+''+description.title), {
                pdf : url,
          ...
          getDownloadURL(ref(storage, id+"Middle"+'/'+description.title+'/video'))
            .then((url) => {
              if(description.video==''){
              updateDoc(doc(db, id+'Middle'+'/'+''+description.title), {
                video : url
